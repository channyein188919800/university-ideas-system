...
<div class="admin-shell">
        ...
        <aside class="admin-sidebar" id="adminSidebar">
            <div class="admin-brand" style="justify-content:center;padding:0.5rem 0 0.8rem;">
                <img src="{{ asset('images/logo1_no_bg.png') }}" alt="University Ideas"
            ...
            <div class="admin-nav-group">
                ...
                <a href="{{ route('staff.dashboard') }}"
                    class="admin-nav-link {{ request()->routeIs('staff.dashboard') ? 'active' : '' }}">
        ...
        <section class="admin-main">
            ...
            <div class="staff-topbar">
                <div>
                    ...
                    <p class="text-muted mb-0">{{ auth()->user()?->last_login_at ? 'Welcome back' : 'Welcome' }}, {{ auth()->user()?->name ?? 'User' }}</p>
                    <small class="text-muted d-block">
                        @if(!auth()->user()?->last_login_at)
                            Welcome! This appears to be your first login.
                        @else
                            Last login: {{ auth()->user()->last_login_at->format('M d, Y h:i A') }}
                        @endif
                    </small>
                </div>
                @if($canSubmitIdea)
            ...
            @if($ideaClosureDate || $finalClosureDate)
                <div class="alert alert-info mb-4">
                    ...
                    @if($ideaClosureDate)
                        Idea submission {{ now()->lt($ideaClosureDate) ? 'closes' : 'closed' }} on
                        <strong>{{ $ideaClosureDate->format('F d, Y') }}</strong>
                    @endif
                    @if($finalClosureDate)
                        | Commenting {{ now()->lt($finalClosureDate) ? 'closes' : 'closed' }} on
                            ...
                            @forelse($myIdeas as $idea)
                                <div class="mb-3 pb-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div>
                                            <span class="badge bg-success me-1"><i class="bi bi-hand-thumbs-up"></i>
                                                {{ $idea->thumbs_up_count }}</span>
                                            <span class="badge bg-primary"><i class="bi bi-chat"></i>
