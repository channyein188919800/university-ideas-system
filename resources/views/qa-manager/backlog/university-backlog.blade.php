    ...
    <div class="qa-card mb-3">
        <div class="qa-card-body">
            <div class="d-flex flex-wrap gap-2">
                <a class="btn btn-sm {{ $tab === 'pending' ? 'btn-primary' : 'btn-outline-primary' }}"
                    href="{{ route('qa-manager.backlog.index', array_merge(request()->query(), ['tab' => 'pending', 'page' => null])) }}">Pending</a>
                <a class="btn btn-sm {{ $tab === 'approved' ? 'btn-primary' : 'btn-outline-primary' }}"
                    href="{{ route('qa-manager.backlog.index', array_merge(request()->query(), ['tab' => 'approved', 'page' => null])) }}">Approved</a>
                <a class="btn btn-sm {{ $tab === 'rejected' ? 'btn-primary' : 'btn-outline-primary' }}"
                    href="{{ route('qa-manager.backlog.index', array_merge(request()->query(), ['tab' => 'rejected', 'page' => null])) }}">Rejected</a>
            </div>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($ideas as $idea)
                            <tr>
                                <td>
                                    ...
                                    <small class="text-muted">#{{ $idea->id }} •
                                        {{ $idea->created_at->diffForHumans() }}</small>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <form method="POST" action="{{ route('qa-manager.ideas.toggle-hidden', $idea) }}">
                                            ...
                                            @method('PATCH')
                                            <button type="submit"
                                                class="btn btn-sm {{ $idea->hidden ? 'btn-success' : 'btn-outline-danger' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-4 text-muted">No backlog items found for current filters.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
